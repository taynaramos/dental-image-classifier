...
from src.pytorch_classifier.predict import Prediction, predict
from src.pytorch_classifier.utils import get_device, load_checkpoint
...
class TorchPredict:
    """Comando ``torch-predict``: classifica uma imagem ou pasta com o modelo CNN (PyTorch)."""

    name: ClassVar[str] = "torch-predict"
    ...
    def __init__(self, subparsers: argparse._SubParsersAction) -> None:
        ...
        parser.add_argument(
            ...
            type=Path,
            required=True,
            help="Caminho para o checkpoint gerado pelo torch-train.",
        )
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        model, classes, config = load_checkpoint(args.model, device)
        ...
        for caminho in caminhos:
            pred = predict(caminho, model, classes, config, device)
