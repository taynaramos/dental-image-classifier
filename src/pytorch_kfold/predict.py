"""Inferência com o classificador CNN (PyTorch) treinado."""
...
def predict(
    ...
    Aplica o mesmo pré-processamento usado no treino (via *config*) e roda a
    rede em modo de avaliação, sem calcular gradientes.
    ...
    config:
        Configuração usada no treino (tamanho da imagem, escala de cinza).
    device:
    ...
    """
    transform = build_transform(config.image_size, config.grayscale)
